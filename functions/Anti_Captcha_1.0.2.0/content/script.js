...
function createSolverButton(captchaType, widgetId) {
    let button = $(`
        <div class="anti-captcha-blocker" data-state="ready" data-captcha-type="${captchaType}" data-widget-id="${widgetId}">
            <div class="anti-captcha-blocker-image">
                <img src="${chrome.runtime.getURL("assets/images/icon_32.png")}">
            </div>
            <div class="anti-captcha-blocker-info">${chrome.i18n.getMessage("solveWith")}</div>
        </div>
    `);
...
function changeSolverButtonState(button, state, message) {
    ...
    button.find(".anti-captcha-blocker-info").text(message);
...
function getSolverButton(captchaType, widgetId) {
    return $(".anti-captcha-blocker[data-captcha-type=" + captchaType + "][data-widget-id=" + widgetId + "]");
}
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    if (request.command == "getContextMenuEl") {
        ...
        if (request.element == 'input') {
            $('.anti-captcha-toast .close').click();
...
function showToast(message) {
    if (!$('body > .anti-captcha-toast-container').length) {
        $('body').append(`<div class="anti-captcha-toast-container"></div>`);
    }

    let toastEl = $(`
        <div class="anti-captcha-toast">
            <div class="anti-captcha-inner">
                <img src="${chrome.runtime.getURL("assets/images/logo.svg")}" class="anti-captcha-toast-logo">
                <div class="anti-captcha-toast-message">${message}</div>            
            </div>            
            <button type="button" class="close">&times;</button>
        </div>
    `).appendTo('.anti-captcha-toast-container');
